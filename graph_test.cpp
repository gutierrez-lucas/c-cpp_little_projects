...
void print_spt(spt s){
    for(int i = 0; i < s.distances.size(); i++){
        ...
        std::cout << std::boolalpha << s.vertices[i] << "," << s.previous_vertex[i] << "]";
    }
    std::cout<<std::endl;
}
...
int find_min(spt s){
    ...
    min = INF;
    for(int i=0; i<s.distances.size();i++){
        if(s.vertices.at(i) == false){          // if this vertex has not been evaluated
...
void shortest_path(graphClass& g, int from, int to){
    ...
    // init every element as un-evaluated and with infinite distance
        my_spt.vertices.push_back(false);
    ...
    for(int i=0; i < size; i++){
        ...
        my_spt.vertices.at(current_v) = true;

        for(int j=0; j < size; j++){
            if(my_spt.vertices.at(j) == false && my_spt.distances.at(current_v) != INF){
...
int main(){
    graphClass my_graph(5, 0);
    my_graph.add_edge(0,1,1);my_graph.add_edge(0,2,2);my_graph.add_edge(0,3,6);
    my_graph.add_edge(1,0,1);my_graph.add_edge(1,4,9);
    my_graph.add_edge(2,0,2);my_graph.add_edge(2,4,5);
    my_graph.add_edge(3,0,6);my_graph.add_edge(3,4,1);
    my_graph.add_edge(4,1,9);my_graph.add_edge(4,2,6);my_graph.add_edge(4,3,1);
